...
if [ "$#" -ne 4 ]; then
    ...
    echo "Script usage:"
    echo "select_files.sh <bam file> <filtering type (1 or 20)> <output directory> <conda_root_directory>"
    echo "---"
    ...
    [ -d $3 ] || mkdir -p $3
    if [ $2 = 1 ]; then
	cov_1=$($4/calculate_genomecov.sh $1 |
	...
	cov_20=$($4/calculate_genomecov.sh $1 |
