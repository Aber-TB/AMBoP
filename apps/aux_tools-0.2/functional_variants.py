...
def main(): 
    ...
    for line in sys.stdin:
        ...
        if line.startswith('#CHROM'): 
            ...
            # use 'replace' to capture multiple alleles as alternatives and stop columns messing up.
            alternate = [line.strip().split("\t")[4].replace(",", ";")]
            # If there is snp-eff info, then get out key parts to keep for the main output file:
            info = line.split("\t")[7]
            # Assume SnpEFF info is absent and set to N/A first, then if it is present in the fields in the vcf, the
            # variable EFF can be overwritten.
        ...
        for var_key, var_value in var_dict.items():
            ...
            for annotation_key, annotation_value in annotations_dict.items():
                ...
                if annotation_key[0] <= int(var_key) <= annotation_key[1]:
                    output = var_key, ",", ",".join(var_value[0:3]), ",", ";".join(var_value[3:-len(eff)]), ",", \
                        ",".join(var_value[-len(eff):]), ",", annotation_value
