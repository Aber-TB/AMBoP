...
if [ "$#" -ne 1 ]; then
    ...
    chosen by user) - percentage of ref genome with at least 1 read coverage and at least 10 read coverage."
    echo "Script usage:"
    echo "calculate_genomecov.sh <bam file>"
    echo ""
    echo "Outputs the percentage of the reference genome with at least 1 read and at least 10 reads covering the site."
	...
	cov_10=$(awk 'BEGIN{cov = 0} {if ($3 >= 10) {cov += 1}} END {print cov/4349904*100}' < $1.genomecov.temp);
	echo -e ${1} "\t" $no_cov "\t" $cov_10  
	rm $1.genomecov.temp;
    fi
fi
